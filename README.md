...
# Crypto-withdrawal guard

Untrusted input is an address and amount; it can only ever become one of a fixed set of decisions over a
...
- **Untrusted input:** `fetch<web>` — an address and amount
- **Closed type:** `type Decision = Send(Wallet) | Reject` where `Wallet` is closed
- **Sink / capability:** `grant withdraw irreversible` → `commit { withdraw(d) }`
- **Consequence axes:** Trust, Reversibility

## Run the demo

```sh
examples/apps/crypto-withdrawal/demo.sh
```
...
- `crypto-withdrawal_safe.eide` — the correct design.
- `crypto-withdrawal_unsafe.eide` — the tempting-but-wrong version (the negative example a model must learn to reject).
